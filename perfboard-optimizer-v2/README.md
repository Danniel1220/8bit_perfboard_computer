# Perfboard optimizer v2 — fresh routing with physical junctions
...
```powershell
python optimize_alu.py --fresh --workers 12
```
...
.\.venv\Scripts\python.exe optimize_alu.py --fresh --workers 12
```
...
Resume is also the default. You may change worker count between sessions. Only one coordinator can write to a run folder; an OS lock prevents collisions. Do not copy an old v1 checkpoint into the v2 run: physical rules differ, and compatibility checks intentionally reject it.
