...
from .portfolio_worker import run_portfolio_worker
...
def _optimize(config,args,run):
    ...
    portfolio=Portfolio(config,run,best,args.workers,args.seed) if config.get('portfolio',{}).get('enabled') else None
    ...
    def publish(reason,new_best=False,worker=None):
        ...
        if portfolio:
            portfolio.persist();portfolio.export(force=reason in ('requested','startup/import','clean shutdown'))
    ...
    try:
        ...
        for wid in range(args.workers):
            ...
            p=ctx.Process(target=run_portfolio_worker if portfolio else run_worker,args=(config,portfolio.starting(wid) if portfolio else best,str(run),wid,args.seed+wid*1000003,outbox,inbox,stop,pause,{'quota':quota,'verbosity':args.verbosity,'profile':args.profile,'session_id':session_id,**(portfolio.settings(wid) if portfolio else {})}),name=f'perfboard-{wid}')
        ...
        while len(done)<len(processes):
            ...
            if now-last_poll>=2:
                last_poll=now
                if portfolio:portfolio.poll();portfolio.export()
            ...
            if not dashboard.enabled and now-last_print>=10:
                if portfolio:
                    for row in portfolio.rows():print(f"{row['id']}: {row['unresolved']} unresolved; merit {row['finish_score']:.1f}; workers {row['workers']}; grace {row['remaining']}",flush=True)
