...
def bundle(run,output=None,tail_bytes=1024*1024):
    ...
    for name in ('config-used.json','settings.json','last-session.json','runtime.json','checkpoints/best.json','exports/validation.json','exports/connections.csv','exports/underneath-connections.csv','exports/physical-wires.json'):
        p=run/name
    ...
    for p in (run/'logs').glob('*.jsonl'):
        ...
        files[p.relative_to(run).as_posix()]=data
    for p in sorted((run/'summaries').glob('*.json'))[-12:]:files[p.relative_to(run).as_posix()]=p.read_bytes()
    for pattern in ('portfolio/state*.json','portfolio/status.json','layouts/*/checkpoints/best.json','layouts/*/exports/validation.json','layouts/*/exports/unfinished-connections.json'):
        for p in run.glob(pattern):files[p.relative_to(run).as_posix()]=p.read_bytes()
    workers=[]
    for p in (run/'workers').glob('*.state.json'):
        try:
            state=read_checkpoint(p);workers.append({k:state[k] for k in ('worker','seed','lifetime_candidates','stats','metrics','history','role','slot','epoch','trial_evaluations','layout_evaluations','baseline_unresolved') if k in state})
