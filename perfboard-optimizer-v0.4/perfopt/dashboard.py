...
def render(state):
    ...
    if 'finish_score' in best:entries.insert(4,('manual-finish merit',f"{best['finish_score']:.2f} (lower is better)"))
    for a,b in entries:table.add_row(a,b)
    layouts=Table('Layout','Open','Merit','Bundle','Group spread','Workers','Evaluated','Grace left','Progress','Last gain',expand=True)
    for row in state.get('layouts',[]):
        layouts.add_row(row['id'],str(row['unresolved']),f"{row['finish_score']:.1f}",f"{row['bundle']:.2f}",f"{row['organization']:.1f}",row['workers'],str(row['evaluations']),str(row['remaining']),str(row['progress']),duration(row['since_progress']))
    workers=Table('Worker','Activity','Scored','c/s','Plateau','CPU s','RSS MiB',expand=True)
    for wid,s in sorted(state['worker_stats'].items()):
        workers.add_row(str(wid),(s.get('role','worker')+' '+s.get('layout_id','')+': '+s.get('activity','starting')+' grace:'+str(s.get('trial_remaining',0))),f"{s.get('candidates',0):,}",f"{s.get('candidates',0)/max(s.get('elapsed',0),.01):.1f}",str(s.get('plateau',0)),f"{s.get('cpu_seconds',0):.1f}",f"{s.get('memory_bytes',0)/1048576:.0f}" if s.get('memory_bytes') else '—')
    phases=state.get('phase_seconds',{});total=sum(phases.values()) or 1
    phase_text=' · '.join(f'{k}: {v/total:.0%}' for k,v in phases.items())
    return Group(Panel(table,title='PERFBOARD OPTIMIZER — '+state['name'],border_style='cyan'),layouts,workers,phase_text,*state.get('events',[])[-4:])
