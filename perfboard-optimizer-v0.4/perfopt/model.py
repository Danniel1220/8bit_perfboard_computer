...
def load_config(path):
    ...
    if portfolio.get('enabled'):
        for key in ('slots','trial_candidates','svg_seconds'):
            if type(portfolio.get(key)) is not int or portfolio[key]<1:raise ValueError(f'portfolio.{key} must be a positive integer')
        if type(portfolio.get('scouts')) is not int or portfolio['scouts']<0:raise ValueError('portfolio.scouts must be nonnegative')
        for key in ('diversity_cells','progress_bonus_cap','progress_weight'):
            if not isinstance(portfolio.get(key),(int,float)) or not math.isfinite(portfolio[key]) or portfolio[key]<0:raise ValueError(f'Invalid portfolio.{key}')
        weights=portfolio.get('merit_weights',{})
        if not weights or any(not isinstance(v,(int,float)) or not math.isfinite(v) or v<0 for v in weights.values()) or not any(weights.values()):raise ValueError('Invalid portfolio merit weights')
        refs={spec['id'] for spec in c['components'] if spec['function']!='connector'};seen=set()
        for name,members in c.get('placement_groups',{}).items():
            if not members or set(members)-refs or len(members)!=len(set(members)) or seen.intersection(members):raise ValueError('Invalid/disjoint placement_groups required')
            seen.update(members)
        if seen!=refs:raise ValueError('Every IC must belong to exactly one placement group')
    ...
    c['_structure_hash'] = digest({k:v for k,v in c.items() if k not in ('search','weights','colors','seed','portfolio')})
