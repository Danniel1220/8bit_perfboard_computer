...
from .exploration import explorer_worker,diversify
...
def run_worker(config,starting,run_dir,worker_id,seed,outbox,inbox,stop,pause,settings):
    ...
    history={};lifetime=0
    explorer=explorer_worker(config,worker_id)
    exploration_remaining=0
    next_exploration=config['search'].get('exploration_initial_candidates',0)
    ...
    try:
        ...
        if not report['metrics']['hard_violations']:
            ...
            exploration_remaining=saved.get('exploration_remaining',0) if explorer else 0
            next_exploration=saved.get('next_exploration',next_exploration)
    ...
    mailbox=Mailbox(outbox,run,worker_id,settings.get('session_id'))
    def send(kind,**payload):
        if not mailbox.send(kind,**payload):stats['dropped_notifications']=stats.get('dropped_notifications',0)+1
    def pulse(what=None,force=False):
        nonlocal last_pulse,activity
        if what:activity=what
        now=time.perf_counter()
        if force or now-last_pulse>=0.25:
            send('telemetry',stats={**stats,'phase_seconds':dict(stats['phase_seconds']),'mutation_counts':dict(stats['mutation_counts']),'mutation_accepted':dict(stats['mutation_accepted']),'route_failures':dict(stats['route_failures']),'activity':activity,'elapsed':now-start,'cpu_seconds':time.process_time(),'memory_bytes':memory_bytes(),'rank':metrics['rank'],'plateau':max(old_level,0),'history_cells':len(history),'explorer':explorer,'exploration_remaining':exploration_remaining})
            last_pulse=now
    def payload(candidate,rank_metrics):
        return {'schema_version':1,'structure_hash':config['_structure_hash'],'session_id':settings.get('session_id'),'candidate':candidate,'metrics':rank_metrics,'rng_state':rng.getstate(),'history':[[n,p,v] for (n,p),v in list(history.items())[-4096:]],'worker':worker_id,'seed':seed,'lifetime_candidates':lifetime+stats['candidates'],'exploration_remaining':exploration_remaining,'next_exploration':next_exploration,'stats':{**stats,'phase_seconds':dict(stats['phase_seconds'])}}
    ...
    try:
        ...
        while not stop.is_set() and (quota is None or stats['candidates']<quota):
            ...
            if pending and not exploration_remaining and now-last_adopt>=config['search']['adopt_seconds']:
                try:
            ...
            if not exploration_remaining and level>=2 and rng.random()<0.025:
            ...
            if explorer and not exploration_remaining and lifetime+stats['candidates']>=next_exploration:
                pulse('exploring new placement',True)
                exploration_start=time.perf_counter()
                alternative,detail=diversify(config,current,rng,stop)
                stats['phase_seconds']['placement_exploration']+=time.perf_counter()-exploration_start
                stats['placement_exploration_attempts']=stats.get('placement_exploration_attempts',0)+1
                next_exploration=lifetime+stats['candidates']+config['search'].get('exploration_interval_candidates',800)
                if alternative is not None:
                    current=alternative;metrics=score(config,current);geometry=Geometry(config,current);history={}
                    stats['placement_exploration_started']=stats.get('placement_exploration_started',0)+1
                    exploration_remaining=config['search'].get('exploration_grace_candidates',300)
                    stats['restarts']+=1
                log.write('placement_exploration',worker=worker_id,started=alternative is not None,grace=exploration_remaining,rank=metrics['rank'],mutation=detail)
            pulse('mutating')
            if exploration_remaining and rng.random()<.85:
                candidate=copy.deepcopy(current);mutation={'type':'exploration_route','components':[]};logical=False
            else:
            ...
            if exploration_remaining:exploration_remaining-=1
        ...
        log.close()
        # All critical state is durable. Never hang process shutdown waiting for
        # an undrained telemetry Queue feeder after the coordinator pauses/exits.
