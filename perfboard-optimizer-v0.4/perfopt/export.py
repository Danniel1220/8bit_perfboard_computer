...
def export_best(config,candidate,report,directory):
    ...
    if config.get('portfolio',{}).get('unfinished_guides',False):
        missing=guides(Router(config,candidate,g))
        layer=element('g',{'id':'Unfinished-connections','data-description':'Visual guides only; not physical wire routes'},root)
        for item in missing:
            ax,ay=g.xy(item['a']);bx,by=g.xy(item['b'])
            x,y,u,v=[(n+.5)*pitch for n in (ax,ay,bx,by)]
            element('path',{'d':f'M {x},{y} L {u},{v}','fill':'none','stroke':'#df168d','stroke-width':.65,'stroke-dasharray':'3 2'},layer)
            for cx,cy in ((x,y),(u,v)):
                element('circle',{'cx':cx,'cy':cy,'r':pitch*.65,'fill':'none','stroke':'#df168d','stroke-width':.8},layer)
            element('text',{'x':(x+u)/2,'y':(y+v)/2-2,'fill':'#df168d','font-size':4,'font-family':'Arial'},layer,text=item['net'])
        atomic_json(directory/'unfinished-connections.json',missing)
