...
from perfopt.portfolio_worker import run_portfolio_worker as run_worker
...
if __name__=='__main__':
    ...
    with tempfile.TemporaryDirectory(prefix='perfopt-full-queue-') as folder:
        ...
        p=ctx.Process(target=run_worker,args=(c,best,folder,0,173688151,outbox,inbox,stop,pause,{'quota':1,'verbosity':'normal','session_id':'queue-test','role':'router','slot':0,'epoch':0}))
        p.start();p.join(8)
        try:
