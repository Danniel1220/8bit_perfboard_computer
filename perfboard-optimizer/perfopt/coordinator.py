...
def _optimize(config,args,run):
    ...
    try:
        ...
        for wid in range(args.workers):
            ...
            p=ctx.Process(target=run_worker,args=(config,best,str(run),wid,args.seed+wid*1000003,outbox,inbox,stop,pause,{'quota':quota,'verbosity':args.verbosity,'profile':args.profile}),name=f'perfboard-{wid}')
        ...
        while len(done)<len(processes):
            ...
            for message in messages:
                ...
                elif kind=='error':errors.append(message['error']);event('worker_error',f'Worker {wid} failed; saving the best before stopping','red',worker=wid,error=message['error']);stop.set();done.add(wid)
